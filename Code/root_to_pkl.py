...
highly_correlated_features = ['Mbbbb_Max', 'HT_all']
...
with open('../Data/data_trg_vs_src', 'wb') as f:
    pickle.dump(list(reversed(data_insts)), f)
    pickle.dump(list(reversed(data_labels)), f)
    pickle.dump(list(reversed(data_name)), f)

print("Data is saved to 'data_trg_vs_src' in pickle format")
############

scaler = StandardScaler()
scaler.fit(np.concatenate((ttH_np[:2*N_ttH_events], ttbar_np[:2*N_bkg_events])))

src1 = coo_matrix(scaler.transform(np.concatenate((ttH_np[N_ttH_events:2*N_ttH_events], ttbar_np[N_bkg_events:2*N_bkg_events])))).tocsc()
